...
from dps.external.rpdiff.utils.torch_util import angle_axis_to_rotation_matrix, transform_pcd_torch
from dps.external.rpdiff.utils.torch3d_util import matrix_to_quaternion, quaternion_to_matrix, matrix_to_axis_angle
...
def single_shot_regression(mc_vis, parent_pcd, child_pcd, rpdiff_dict, policy_model, 
    ...
    parent_latent = parent_dps.external.rpdiff.model.extract_latent(parent_rpdiff_mi).detach()  # assumes we have already centered based on the parent
    parent_rpdiff_child_desc = parent_dps.external.rpdiff.model.forward_latent(parent_latent, parent_rpdiff_mi['coords']).detach()

    child_latent = child_dps.external.rpdiff.model.extract_latent(child_rpdiff_mi).detach()  # assumes we have already centered based on the child
    child_rpdiff_parent_desc = child_dps.external.rpdiff.model.forward_latent(child_latent, child_rpdiff_mi['coords']).detach()
...
def single_shot_regression_transformer_combine(mc_vis, parent_pcd, child_pcd, rpdiff_dict, policy_model_dict, 
    ...
    parent_latent = parent_dps.external.rpdiff.model.extract_latent(parent_rpdiff_mi).detach()  # assumes we have already centered based on the parent
    parent_rpdiff_desc = parent_dps.external.rpdiff.model.forward_latent(parent_latent, parent_rpdiff_mi['coords']).detach()

    child_latent = child_dps.external.rpdiff.model.extract_latent(child_rpdiff_mi).detach()  # assumes we have already centered based on the child
    child_rpdiff_desc = child_dps.external.rpdiff.model.forward_latent(child_latent, child_rpdiff_mi['coords']).detach()
    ...
    parent_rot_latent = parent_dps.external.rpdiff.model.extract_latent(parent_rpdiff_mi).detach()  
    parent_rpdiff_rot_desc = parent_dps.external.rpdiff.model.forward_latent(parent_rot_latent, parent_rpdiff_mi['coords']).detach()
    child_rot_latent = child_dps.external.rpdiff.model.extract_latent(child_rpdiff_mi).detach()  
    child_rpdiff_rot_desc = child_dps.external.rpdiff.model.forward_latent(child_rot_latent, child_rpdiff_mi['coords']).detach()
