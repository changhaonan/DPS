...
from dps.external.rpdiff.share.globals import bad_shapenet_mug_ids_list, bad_shapenet_bowls_ids_list, bad_shapenet_bottles_ids_list
...
from dps.external.rpdiff.utils.eval_gen_utils import constraint_obj_world, safeCollisionFilterPair, safeRemoveConstraint

from dps.external.rpdiff.utils.relational_policy.multistep_pose_regression import policy_inference_methods_dict
from dps.external.rpdiff.model.coarse_affordance import CoarseAffordanceVoxelRot
from dps.external.rpdiff.model.transformer.policy import NSMTransformerSingleTransformationRegression, NSMTransformerSingleTransformationRegressionCVAE, NSMTransformerSingleSuccessClassifier
    ...
    for iteration in range(exp_args.start_iteration, exp_args.num_iterations):
        ...
        for pc in pcl:
            ...
            for obj_ind, obj_file in enumerate(obj_file_list):
                ...
                while True:
                    if object_class in ["syn_bookshelf", "syn_cabinet", "syn_cabinet_packed_nonuniform", "syn_cabinet_packed_uniform"]:
                        ...
                        if load_pose_type == "any_pose":
                            ...
                            pos = [np.random.random() * (x_high - x_low) + x_low, np.random.random() * (y_high - y_low) + y_low, table_z + 0.02]
                            pose = pos + ori
                ...
                if object_class in concave_urdf_classes:
                    ...
                    obj_id = pb_client.load_geom("mesh", mass=mass, mesh_scale=mesh_scale, visualfile=obj_obj_file_dec, collifile=obj_obj_file_dec, base_pos=pos, base_ori=ori, rgba=color)
        ...
        if (eval_bookshelf_task or eval_cabinet_task or eval_mug_rack_multi_task) and exp_args.use_floating_hand_execution:
            ...
            while True:
                out_move_until_touch = floating_hand.hand.move_ee_xyz_until_touch(
                    hand_delta_pos_to_exec, eef_step=0.001, coll_id_pairs=((place_parent_obj_id, -1), (child_obj_id, -1)), use_force=True, force_thresh=15.0, return_stop_motion=True
                )
        ...
        if not exp_args.use_floating_hand_execution:
            ...
            final_child_pose_parent = util.convert_reference_frame(
                pose_source=util.pose_from_matrix(final_child_pose_mat), pose_frame_target=util.pose_from_matrix(start_parent_pose_mat), pose_frame_source=util.unit_pose()
            )
            ...
            final_child_pose_upside_down = util.convert_reference_frame(
                pose_source=final_child_pose_parent, pose_frame_target=util.unit_pose(), pose_frame_source=util.pose_from_matrix(upside_down_pose_mat)
            )
