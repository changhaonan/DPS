...
def order_preserved_unique_torch(x, return_inverse=False):
    unique, inverse = torch.unique(x, sorted=True, return_inverse=True)
    perm = torch.arange(inverse.size(0), dtype=inverse.dtype, device=inverse.device)
    inverse, perm = inverse.flip([0]), perm.flip([0])
    ind = inverse.new_empty(unique.size(0)).scatter_(0, inverse, perm)

    ind = torch.argsort(ind)
    unique = unique[ind]
    return unique
...
class DualSoftmaxReposition(nn.Module):
    def __init__(
        ...
        self.focal_gamma = focal_gamma  # 对于样本难度的加权系数 (见 focal loss paper Fig.1)
        self.detach = detach
...
class KnnAttentionPool(nn.Module):
    ...
    def forward(self, coord, feat, offset):
        ...
        pooled_feat, knn_indexes = resample(
            pooled_coord,
            coord,
            feat,
            k=self.k,
            query_offset=pooled_offset,
            base_offset=offset,
            return_knn_indexes=True,
...
class KnnTransformerNetwork(BaseTransformerNetwork):
    def forward(self, feat={}, coord={}, batch_index={}, knn_indexes={}, embedding={}, collect_cross=False):
        ...
        for layer in self.layer_types:
            ...
            if layer_name == "self":
                ...
                for block, (a, b) in zip(knn_t_block, split_list_into_groups(self.parse_interaction_roles(layer), 2)):
                    ...
                    tmp_result[a] = block(
                        ...
                        coord=coord[a],
                        context_feat=feat[b],
                        context_coord=coord[b],
                        knn_indexes=knn_indexes[f"{ka}2{kb}"],
                    )
                ...
                position_layer_outputs.append({"R": R, "t": t, **others})
                ...
                if not self.skip_repo:
                    ...
                    knn_indexes[f"{a}2{b}"], _ = knn(
                        coord_a,
                        coord_b,
                        k_a2b,
                        query_offset=batch2offset(batch_index[a]),
                        base_offset=batch2offset(batch_index[b]),
                    )
                    knn_indexes[f"{b}2{a}"], _ = knn(
                        coord_b,
                        coord_a,
                        k_b2a,
                        query_offset=batch2offset(batch_index[b]),
                        base_offset=batch2offset(batch_index[a]),
                    )
            ...
            if layer_name not in self.NON_MODULE_LAYERS:
                layer_id += 1
