"""PointCloud Diffusion Network For Segmentation (Version 2).
This version doesn't use superpoint
"""

from __future__ import annotations
import torch
import torch.nn as nn
from dps.model.network.geometric import PointTransformerNetwork, to_dense_batch, to_flat_batch, batch2offset, offset2batch, knn, PointBatchNorm, PointTransformer
...
class PositionEmbeddingCoordsSine(nn.Module):
    """Position embedding from Mask3D"""
...
class PointSAMBlock(nn.Module):
    """Two-direction attention block like SAM"""
...
class PcdSAMNoiseNet(nn.Module):
    ...
    def __init__(self, grid_sizes, depths, dec_depths, hidden_dims, n_heads, ks, in_dim, **kwargs) -> None:
        super().__init__()
        num_sam_blocks = kwargs.get("num_sam_blocks", 2)
        ...
        for i in range(num_sam_blocks):
            self.sam_blocks.append(PointSAMBlock(hidden_dim=hidden_dims[1], n_heads=n_heads[1], dropout=0.1))
    ...
    def encode_pcd(self, points: list[torch.Tensor], attrs=None) -> torch.Tensor:
        ...
        return points, cluster_indexes, attrs
    ...
    def forward(self, noisy_t: torch.Tensor, points: list[torch.Tensor], t: int) -> torch.Tensor:
        ...
        coord, feat, offset = points
        knn_indices = dict()
        knn_indices["query2query"] = knn(query=coord, base=coord, query_offset=offset, base_offset=offset, k=self.seg_knn)[0]
        knn_indices["query2context"] = knn_indices["query2query"]
        knn_indices["context2query"] = knn_indices["query2query"]

        noisy_t = self.proj_up(noisy_t)
        noisy_points = [coord, noisy_t, offset]
        for sam_block in self.sam_blocks:
            noisy_points, points = sam_block(noisy_points, points, knn_indices)
        noisy_t = noisy_points[1]
        ...
        coord, feat, offset = points
        batch_index = offset2batch(offset)
        coord = to_dense_batch(coord, batch_index)[0]
        noisy_t, mask = to_dense_batch(noisy_t, batch_index)
        padding_mask = mask == 0
